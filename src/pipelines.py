...
This module contains the functions to generate tensorflow pipelines ready 
to feed the captioning models. 

It requires the following to work:

   1. The raw folder has the unzipped datasets. This is done by the script
      :py:mod:`download_data.py <download_data>` .
   2. The prepared datasets are ready. These are generated by the script
      :py:mod:`prepare_data.py <prepare_data>` .
...
def build_tf_dataset(df_subset,image_size):
    ...
    images_tf_dataset = (
                            tf.data.Dataset
                            .from_tensor_slices(df_subset['Path'])
                            .map(lambda x:decode_and_resize(x,image_size),
...
def build_pipeline(inter_dataset_path='../data/intermediate/inter_medpix.csv',
    ...
    val_df=df.loc[df.Split=='validation']
    test_df=df.loc[df.Split=='test']

    # Generate tensorflow datasets for training, validation and test sets.-----
    # Generate training tensorflow dataset from training df
    train_captioning_tf_dataset = build_tf_dataset(train_df,image_size) 
    # Generate validation tensorflow dataset from validation df
    val_captioning_tf_dataset = build_tf_dataset(val_df,image_size) 
    # Generate test tensorflow dataset from test df
    test_captioning_tf_dataset = build_tf_dataset(test_df,image_size)   

    pipeline = {'captioning':{
                                'train':train_captioning_tf_dataset,
                                'val':val_captioning_tf_dataset,
                                'test':test_captioning_tf_dataset,
                                'train_captions':(train_df['Full_Caption']
                                                  .astype(str).to_list())
                             }
                } 
    return pipeline
...
#             # Output for debugging train and test sets:
...
def build_coco_pipeline(downscale=False,image_size=(299,299),tokenizer=None):
    ...
    Returns:
        dictionary: A dictionary containing tensorflow datasets.
