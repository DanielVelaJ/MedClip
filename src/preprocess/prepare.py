...
def make_model_training_files(inter_dataset=None, inter_dataset_path=None):
    """
    ...
    generates 3 files. Two files to train the model and one file that
    contains metadata about lables. The files are described as follows:

        1.labels.pkl: a python dictionary that contains filepaths relative
            to the src directory and the corresponding label vector. Ex:
                {'data/raw/medpix/imgages/img0.jpg':[1,0,0,0,0,1],
                 'data/raw/medpix/images/img1.jpg':[1,0,0,1,1,1],
                 ...}
        2.captions.pkl: a python dictionary that containes filepaths relative
            to the src directory and the corresponding caption string. Ex:
                {'data/raw/medpix/imgages/img0.jpg': "a messed up left knee",
                 'data/raw/medpix/images/img1.jpg': "this guy is ...healthy?",
                 ...}
        3.label_description: A dictionary containing the mapping between the
            index of the label vector and the meaning of each label. Ex:
                {0: ctscan
                 1: MRI
                 3: Lateral
                 4: Frontal
                 6: Pneumonia_present,
                 ...}
    The objects described above are saved in the data/model_ready/ folder and
    returned by the function
        inter_dataset (pandas df): a inter_dataset dataframe generated by the
            other functions in this module that carry dataset names such as
            'chexpert()','medpix()', etc.
        inter_dataset_path (str): a relative path from src to an
            inter_dataframe. These are located in the 'data/intermediate/'
            durectirt

    Returns:
        labels(python dict): a python dictionary that contains filepaths
        relative to the src directory and the corresponding label vector. Ex:
                {'data/raw/medpix/imgages/img0.jpg':[1,0,0,0,0,1],
                 'data/raw/medpix/images/img1.jpg':[1,0,0,1,1,1],
                 ...}
        captions (python dict):  a python dictionary that containes filepaths
        relative to the src directory and the corresponding caption string. Ex:
                {'data/raw/medpix/imgages/img0.jpg': "a messed up left knee",
                 'data/raw/medpix/images/img1.jpg': "this guy is ...healthy?",
                 ...}
        label_description (python dict): A dictionary containing the mapping
            between the index of the label vector and the meaning of each
            label. Ex:
                {0: ctscan
                 1: MRI
                 3: Lateral
    ...
    """
    if inter_dataset is None:
        inter_dataset = pd.read_pickle(inter_dataset_path)

    # Get lists of paths to images:
    image_paths = inter_dataset['Path']
